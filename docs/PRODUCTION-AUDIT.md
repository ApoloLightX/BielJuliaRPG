...
Esta auditoria compara a branch de hardening com a `main` e registra apenas estados verificados. Alterações de banco descritas como "migration pronta" ainda precisam ser aplicadas e verificadas no projeto Supabase real antes do merge.
...
| SAVE-007 | P1 | Dados | save cloud podia sobrescrever silenciosamente outro aparelho | revisão CAS + fila local + conflito explícito | migration pronta / cliente testado |
| DB-008 | P1 | Supabase | duas entradas simultâneas podiam exceder limite de 2 membros | `FOR UPDATE` no RPC de entrada | migration pronta |
| DEP-009 | P1 | Dependências | Vite/esbuild com vulnerabilidades conhecidas | Vite 8.2.1; `npm audit` retorna 0 | testado |
| INFRA-010 | P1 | Supabase | schema real do projeto não pôde ser verificado/aplicado nesta sessão | migrations versionadas e runbook preparado | pendente por dependência externa |
...
| RLS-015 | P2 | Supabase | escrita direta do estado e funções com grants mais amplos | escrita por RPC CAS, revokes explícitos e search_path endurecido | migration pronta |
...
| RESOURCE-019 | P2 | Supabase | conta autenticada podia criar campanhas sem quota | policy RLS limita a 20 campanhas por owner | migration pronta |
...
Nenhuma migration alvo contém `USING (true)` ou `WITH CHECK (true)`. Há testes estáticos para esses invariantes.
...
1. As migrations não foram confirmadas como aplicadas no Supabase real.
2. Os advisors de segurança/performance do Supabase não foram executados após as migrations.
