      ...
      .on(
        ...
        (payload) => {
          ...
          if (dirtyRef.current) {
            ...
            setError(
              "Outro aparelho atualizou a campanha enquanto você tinha mudanças locais. Reabra a campanha para sincronizar sem sobrescrever."
            );
            return;
          }
          try {
    ...
    <div className="min-h-screen w-full bg-[#0e0b0a] text-[#e8ddd0] flex flex-col font-serif">
      <header className="border-b border-[#2a1f1a] bg-[#120e0c] px-4 py-3 flex items-center justify-between gap-3 bg-noise">
        <div className="flex items-center gap-2 min-w-0">
          ...
          <div className="min-w-0"><h1 className="font-display text-sm sm:text-base tracking-wide truncate">{campaign.name}</h1><p className="text-[10px] text-[#75655b] tracking-wider">Código {campaign.join_code}</p></div>
        </div>
