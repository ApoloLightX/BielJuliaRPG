...
export default function GameSession({ campaign, userId, onExit }) {
  ...
  const currentSnapshot = useMemo(
    () => sanitizeSnapshot({ schemaVersion: GAME_SCHEMA_VERSION, phase, players, messages, pendingTests, levelUpQueue, revealedRegions, currentRegion, combat, view }),
    [phase, players, messages, pendingTests, levelUpQueue, revealedRegions, currentRegion, combat, view]
  );
  ...
  useEffect(() => {
    ...
    async function loadState() {
      const { data, error: loadError } = await supabase.from("campaign_state").select("state,updated_at,updated_by,revision").eq("campaign_id", campaign.id).single();
      if (!active) return;
    ...
    loadState();
    const channel = supabase.channel(`campaign-state-${campaign.id}`).on("postgres_changes", {
      event: "UPDATE", schema: "public", table: "campaign_state", filter: `campaign_id=eq.${campaign.id}`,
    }, (payload) => {
      ...
      } catch { setError("Foi recebida uma atualização remota inválida."); }
    }).subscribe((status) => {
  ...
  function handleP2Confirm(character) {
    try {
      ...
      if (players[0]?.nick.toLocaleLowerCase() === nextPlayer.nick.toLocaleLowerCase()) { setError("Os dois personagens precisam de nomes diferentes."); return; }
  ...
  function applyMasterDirectives(reply) {
    ...
    if (directives.damageMatches.length || directives.healMatches.length) nextPlayers = applyHpEffects(nextPlayers, directives.damageMatches, directives.healMatches);
  ...
  async function sendMessage(text) {
    ...
    try {
      ...
      const response = await fetch("/api/mestre", {
        ...
        body: JSON.stringify({ campaignId: campaign.id, messages: newMessages, players, combat }),
      });
      const data = await response.json().catch(() => ({}));
      if (!response.ok) throw new Error(data?.error || `Erro ${response.status}`);
      applyMasterDirectives(data.reply);
    } catch (requestError) {
  ...
  if (!hydrated) return <div className="min-h-screen bg-[#0e0b0a] text-[#e8ddd0] flex items-center justify-center font-serif"><div className="text-center space-y-2"><Cloud size={26} className="mx-auto text-[#b8492f] animate-pulse" /><p className="text-sm text-[#8a7a6d]">Carregando campanha salva...</p></div></div>;
  ...
  return (
    <div className="min-h-screen w-full bg-[#0e0b0a] text-[#e8ddd0] flex flex-col font-serif">
      <header className="border-b border-[#2a1f1a] bg-[#120e0c] px-4 py-3 flex items-center justify-between gap-3 bg-noise">
        <div className="flex items-center gap-2 min-w-0"><button type="button" onClick={exitToCampaigns} className="text-[#8f7b6e] hover:text-[#e8ddd0]" aria-label="Voltar às campanhas"><ArrowLeft size={18} /></button><Skull size={20} className="ember flex-shrink-0" /><div className="min-w-0"><h1 className="font-display text-sm sm:text-base tracking-wide truncate">{campaign.name}</h1><p className="text-[10px] text-[#75655b] tracking-wider">Código {campaign.join_code}</p></div></div>
        <div className="flex items-center gap-2 flex-shrink-0"><span className={`text-[10px] ${saveStatus.includes("erro") || saveStatus === "conflito" ? "text-[#d26b54]" : "text-[#8b7b70]"}`}>{saveStatus}</span><button type="button" onClick={() => enqueueSave(currentSnapshot)} disabled={saveStatus === "conflito"} className="text-[#9d897c] hover:text-[#e8ddd0] disabled:opacity-40" aria-label="Salvar agora"><Save size={16} /></button></div>
      </header>
      ...
      {phase === "game" && (
        <>
          ...
          <nav className="flex border-b border-[#2a1f1a] bg-[#120e0c]" aria-label="Visões da campanha"><button type="button" onClick={() => setView("chat")} className={`flex-1 flex items-center justify-center gap-1.5 py-2.5 text-xs uppercase tracking-wider font-display ${view === "chat" ? "text-[#e8ddd0] border-b-2 border-[#b8492f]" : "text-[#8a7a6d]"}`}><MessageSquare size={13} /> Mesa</button><button type="button" onClick={() => setView("map")} className={`flex-1 flex items-center justify-center gap-1.5 py-2.5 text-xs uppercase tracking-wider font-display ${view === "map" ? "text-[#e8ddd0] border-b-2 border-[#b8492f]" : "text-[#8a7a6d]"}`}><MapIcon size={13} /> Mapa</button><button type="button" onClick={() => setView("sheet")} className={`flex-1 flex items-center justify-center gap-1.5 py-2.5 text-xs uppercase tracking-wider font-display ${view === "sheet" ? "text-[#e8ddd0] border-b-2 border-[#b8492f]" : "text-[#8a7a6d]"}`}><Backpack size={13} /> Fichas</button></nav>
          {view === "map" ? <div className="flex-1 overflow-y-auto px-5 py-6 bg-noise"><CampaignMap revealedIds={revealedRegions} currentId={currentRegion} /></div> : view === "sheet" ? <div className="flex-1 overflow-y-auto px-4 sm:px-5 py-5 bg-noise"><div className="grid gap-4 md:grid-cols-2 max-w-4xl mx-auto">{players.map((player, index) => player ? <CharacterSheet key={player.nick} player={player} onChangeHp={(delta) => changeHp(index, delta)} onAddItem={(item) => addItem(index, item)} onRemoveItem={(itemIndex) => removeItem(index, itemIndex)} /> : null)}</div></div> : (
            <><main ref={scrollRef} className="flex-1 overflow-y-auto px-4 sm:px-5 py-6 space-y-5 bg-noise">{combat && <CombatTracker combat={combat} players={players} />}{messages.length === 0 && <div className="text-center py-16 space-y-3"><Flame size={28} className="ember mx-auto opacity-60" /><p className="text-[#8a7a6d] text-sm max-w-sm mx-auto leading-relaxed">Os personagens estão prontos. Escreva “vamos começar” para o mestre abrir a campanha.</p></div>}{messages.map((message, index) => <div key={index} className={`flex ${message.role === "user" ? "justify-end" : "justify-start"}`}><div className={`max-w-[85%] rounded px-4 py-3 text-[15px] leading-relaxed whitespace-pre-wrap ${message.role === "user" ? "bg-[#2a1f1a]" : "bg-[#1a1310] border border-[#2a1f1a] text-[#d9cbb8]"}`}>{message.role === "model" && <div className="flex items-center gap-2 mb-1.5 text-xs ember tracking-wider uppercase font-display"><ScrollText size={12} /> Mestre</div>}{message.text}</div></div>)}{loading && <div className="flex justify-start"><div className="bg-[#1a1310] border border-[#2a1f1a] rounded px-4 py-3 text-sm text-[#8a7a6d] italic">O mestre está tecendo o destino de vocês...</div></div>}{error && <div className="text-center"><p className="text-xs text-[#d26b54] bg-[#1a1310] inline-block px-3 py-2 rounded border border-[#3a2419]">{error}</p></div>}{pendingTests.map((nick) => { const player = players.find((candidate) => candidate?.nick === nick); return player ? <DiceRoller key={nick} player={player} onRoll={handleDiceResult} /> : null; })}</main><footer className="border-t border-[#2a1f1a] bg-[#120e0c] px-4 sm:px-5 py-4"><div className="flex gap-2 items-end"><textarea value={input} onChange={(event) => setInput(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter" && !event.shiftKey) { event.preventDefault(); sendMessage(input); } }} placeholder={combat ? "Descrevam a ação de cada personagem no turno" : "O que vocês fazem?"} rows={1} maxLength={3000} className="flex-1 bg-[#0e0b0a] border border-[#3a2a24] rounded px-3 py-2.5 text-sm placeholder-[#5a4d43] focus:outline-none focus:border-[#b8492f] resize-none" /><button type="button" onClick={() => sendMessage(input)} disabled={loading || !input.trim() || saveStatus === "conflito"} className="bg-[#7a2419] hover:bg-[#8e2c1f] disabled:opacity-40 text-[#f0e6da] p-2.5 rounded" aria-label="Enviar"><Send size={18} /></button></div></footer></>
          )}
