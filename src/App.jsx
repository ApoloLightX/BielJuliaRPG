...
export default function App({ onExit }) {
  ...
  useEffect(() => {
    try {
      ...
      if (raw) { applySnapshot(JSON.parse(raw)); setSaveStatus("restaurado"); }
      else setSaveStatus("novo");
    } catch (restoreError) {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      try { localStorage.setItem(STORAGE_KEY, JSON.stringify(currentSnapshot)); setSaveStatus("salvo"); }
      catch (saveError) { console.error("Falha ao salvar localmente", saveError); setSaveStatus("erro no save"); }
    }, 350);
  ...
  async function exportSave() {
    try {
      ...
      } else window.prompt("Copie este código de save:", code);
    } catch (exportError) { setError(`Não foi possível exportar o save: ${exportError.message}`); }
  }
  ...
  function importSave() {
    ...
    } catch { setError("Código de save inválido, corrompido ou incompatível."); }
  }
  ...
  async function sendMessage(text) {
    ...
    setPendingTests([]);
    try {
      const response = await fetch("/api/mestre", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ messages: newMessages, players, combat }) });
    ...
    <div className="min-h-screen w-full bg-[#0e0b0a] text-[#e8ddd0] flex flex-col font-serif">
      ...
      {phase === "game" && (
        <>
          ...
          <nav className="flex border-b border-[#2a1f1a] bg-[#120e0c]" aria-label="Visões da campanha">
            <button type="button" onClick={() => setView("chat")} className={`flex-1 flex items-center justify-center gap-1.5 py-2.5 text-xs uppercase tracking-wider font-display ${view === "chat" ? "text-[#e8ddd0] border-b-2 border-[#b8492f]" : "text-[#8a7a6d]"}`}><MessageSquare size={13} /> Mesa</button>
            <button type="button" onClick={() => setView("map")} className={`flex-1 flex items-center justify-center gap-1.5 py-2.5 text-xs uppercase tracking-wider font-display ${view === "map" ? "text-[#e8ddd0] border-b-2 border-[#b8492f]" : "text-[#8a7a6d]"}`}><MapIcon size={13} /> Mapa</button>
            <button type="button" onClick={() => setView("sheet")} className={`flex-1 flex items-center justify-center gap-1.5 py-2.5 text-xs uppercase tracking-wider font-display ${view === "sheet" ? "text-[#e8ddd0] border-b-2 border-[#b8492f]" : "text-[#8a7a6d]"}`}><Backpack size={13} /> Fichas</button>
          </nav>
          {view === "map" ? <div className="flex-1 overflow-y-auto px-5 py-6 bg-noise"><CampaignMap revealedIds={revealedRegions} currentId={currentRegion} /></div> : view === "sheet" ? <div className="flex-1 overflow-y-auto px-4 sm:px-5 py-5 bg-noise"><div className="grid gap-4 md:grid-cols-2 max-w-4xl mx-auto">{players.map((player, index) => player ? <CharacterSheet key={player.nick} player={player} onChangeHp={(delta) => changeHp(index, delta)} onAddItem={(item) => addItem(index, item)} onRemoveItem={(itemIndex) => removeItem(index, itemIndex)} /> : null)}</div></div> : (
            <>
              <main ref={scrollRef} className="flex-1 overflow-y-auto px-4 sm:px-5 py-6 space-y-5 bg-noise">
                {combat && <CombatTracker combat={combat} players={players} />}
