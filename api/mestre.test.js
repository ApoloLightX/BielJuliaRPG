...
describe("mestre request hardening", () => {
  ...
  it("sanitiza combate, rodada e turno contra jogadores conhecidos", () => {
  ...
  it("sanitiza diário e remove duplicatas", () => {
    const journal = sanitizeJournal({
      summary: "Resumo",
      objective: "Entrar em Vharnak",
      clues: ["Sino mudo", "sino mudo", "x".repeat(500)],
      npcs: [
        { name: "Sella", description: "Sobrevivente" },
        { name: "sella", description: "duplicada" },
      ],
      decisions: ["Esperar a noite"],
    });
    expect(journal.clues).toHaveLength(2);
    expect(journal.clues[1]).toHaveLength(300);
    expect(journal.npcs).toEqual([{ name: "Sella", description: "Sobrevivente" }]);
  });

  it("inclui HP, turno e diário canônico no prompt", () => {
    const prompt = buildSystemPrompt(
      ...
      ],
      {
        ...
        currentTurnIndex: 1,
        order: [
          { type: "enemy", name: "Lobo", hp: 6, maxHp: 12 },
          { type: "player", name: "Biel" },
        ],
      },
      {
        summary: "A dupla chegou à névoa.",
        objective: "Abrir as portas",
        clues: ["O sino não produz som"],
        npcs: [{ name: "Sella", description: "Sobrevivente silenciosa" }],
        decisions: ["Não tocar o sino"],
      }
    );
    ...
    expect(prompt).toContain("Rodada: 2");
    expect(prompt).toContain("Turno atual: Biel");
    expect(prompt).toContain("Lanterna");
    expect(prompt).toContain("DIÁRIO CANÔNICO DA CAMPANHA");
    expect(prompt).toContain("Abrir as portas");
    expect(prompt).toContain("O sino não produz som");
    expect(prompt).toContain("[[OBJETIVO: texto]]");
  });
